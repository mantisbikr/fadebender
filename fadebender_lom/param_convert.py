...
def to_display(value: float, learned_param: Dict[str, Any]) -> Tuple[str, Optional[float]]:
    """Convert normalized value to display string and numeric display when available.

    learned_param may include: min, max, unit, control_type, label_map, samples[], fit{}
    """
    ...
    # Label-based for binary/quantized
    ...
    # Continuous: fit forward to display number
    ...
    # Interpolate samples if present
    if samples:
        ...
        if lo and hi:
            ...
            # Prefer numeric displays when available
...
def to_normalized(target_display_num: float, learned_param: Dict[str, Any]) -> float:
    ...
    # Fallback to nearest sample by display_num
    samples = learned_param.get("samples") or []
    best = None; bestd = 1e9
    for s in samples:
